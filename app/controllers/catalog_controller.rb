...
class CatalogController < ApplicationController  
  ...
  configure_blacklight do |config|
    ...
    config.add_facet_field '695a_filter_sm', :label => :filter_canon_type, :limit => 10
    config.add_facet_field '695e_filter_sm', :label => :filter_interval, :limit => 10
    config.add_facet_field '695g_filter_sm', :label => :filter_temporal_offset, :limit => 10
    config.add_facet_field '695h_filter_sm', :label => :filter_offset_units, :limit => 10
    ...
    config.add_search_field("canon_type") do |field|
      field.label = :filter_canon_type
      field.solr_parameters = { :qf => "695a_text" }
    end

    config.add_search_field("canon_imitation_interval") do |field|
      field.label = :filter_interval
      field.solr_parameters = { :qf => "695e_text" }
    end

    config.add_search_field("canon_temporal_offset") do |field|
      field.label = :filter_temporal_offset
      field.include_in_simple_select = false
      field.solr_parameters = { :qf => "695g_text" }
    end

    config.add_search_field("canon_offset_units") do |field|
      field.label = :filter_offset_units
      field.include_in_simple_select = false
      field.solr_parameters = { :qf => "695h_text" }
    end
